...
describe 'merchants/find api' do
  ...
  it 'returns all merchants with search word in the name, case insensitive' do
    get('/api/v1/merchants/find?name=group')
    ...
    expect(merchant[:data][:attributes][:name]).to eq("Hartmann Group")
  end

  it 'returns empty data hash if there is no result' do
    get('/api/v1/merchants/find?name=ailsuefgliasugf')
